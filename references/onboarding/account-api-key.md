...
Use this shared step before configuring any StudioTwin connector. The user performs account and credential actions; the agent guides them and verifies only what a connected surface reports.
...
No application form or waiting list is required.
...
4. Copy the secret immediately. It is shown only once and cannot be retrieved again. StudioTwin API keys begin with `st_` and belong to the user's organization.
...
- If rejected, confirm the key begins with `st_`, is active, belongs to the expected organization, and contains no copied whitespace.

Continue with the connector-specific setup selected in [../setup.md](../setup.md).