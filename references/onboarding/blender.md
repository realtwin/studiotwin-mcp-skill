...
When the target is an already-open interactive Blender scene, do not launch a
second Blender process—including a headless or background process—for setup or
verification. Verify only through the connected official Blender MCP bridge and
require the intended scene to report `background: false`.
...
3. **Connect the official MCP server.** Use the active MCP client's supported route: the latest `.mcpb` from the official [releases](https://projects.blender.org/lab/blender_mcp/releases), or the official [stdio source setup](https://projects.blender.org/lab/blender_mcp/wiki/Setup) using `uv --directory <clone-path>/mcp run blender-mcp`. When defaults change, keep `BLENDER_MCP_HOST` and `BLENDER_MCP_PORT` consistent with the Blender bridge. If the newly configured tools do not become available through reconnect or hot-load, ask the user to restart or reconnect the current agent harness session and resume the same task or conversation.
4. **Check bridge reachability.** Verify directly—or ask the user to confirm—that the local Blender MCP bridge is reachable from the environment where the agent harness is running, including across WSL, containers, VMs, remote machines, or other network namespaces. `localhost` refers to the harness environment and may not reach Blender across such a boundary. Keep the bridge loopback-only; if it is unreachable, report the boundary rather than exposing the bridge to a network.
5. **Install the StudioTwin Blender addon.** Download the current `.zip` from the [StudioTwin Blender addon releases page](https://github.com/realtwin/studiotwin-blender-addon/releases). In **Preferences → Get Extensions**, choose **Install from Disk**, select the `.zip`, enable **StudioTwin**, and grant its declared local-file permission. The addon is not an MCP server, makes no network requests, and stores no API key.
6. **Verify official Blender MCP.** Call a read-only summary tool, then use `execute_blender_code` to return `bpy.app.background` and the Blender version. The intended open scene must report `background: false`.
7. **Verify the addon.** Through a read-only code call, discover the enabled addon module named `studiotwin` or ending with `.studiotwin`, import it, and confirm `apply_environment_map`, `import_model`, `import_material`, and `import_audio` are callable.
